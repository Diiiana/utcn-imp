...
std::shared_ptr<IfStmt> Parser::ParseIfStmt()
{
  ...
  auto elseCond = ParseElseStmt();
  return std::make_shared<IfStmt>(cond, stmt, elseCond);
}

// ---------------------------------------------SET 2----------------------------
std::shared_ptr<ElseStmt> Parser::ParseElseStmt()
{
  Check(Token::Kind::ELSE);
  lexer_.Next();
  auto stmt = ParseStmt();
  return std::make_shared<ElseStmt>(stmt);
}
...
std::shared_ptr<Expr> Parser::ParseAddSubExpr()
{
  ...
  char operationType;
  while (Current().Is(Token::Kind::PLUS) || Current().Is(Token::Kind::SUBTRACT))
  {
    ...
    {
      if (Current().Is(Token::Kind::SUBTRACT))
      {
        operationType = '-';
      }
    }
    ...
    else
    {
      if (operationType == '-')
      {
        term = std::make_shared<BinaryExpr>(BinaryExpr::Kind::SUBTRACT, term, rhs);
      }
    }
  }
  return term;
...
std::shared_ptr<Expr> Parser::ParseMulDivRemExpr()
{
  ...
  while (Current().Is(Token::Kind::MULTIPLY) || Current().Is(Token::Kind::DIVIDE) || Current().Is(Token::Kind::MODULO))
  {
    ...
    else
    {
      if (operationType == '/')
      {
        term = std::make_shared<BinaryExpr>(BinaryExpr::Kind::DIVIDE, term, rhs);
      }
      else
      {
        if (operationType == '%')
