...
void Codegen::LowerStmt(const Scope &scope, const Stmt &stmt)
{
  ...
  case Stmt::Kind::ELSE:
  {
    return LowerElseStmt(scope, static_cast<const ElseStmt &>(stmt));
  }
  case Stmt::Kind::EXPR:
  {
...
// -----------------------------------------------SET 2--------------------------
void Codegen::LowerElseStmt(const Scope &scope, const ElseStmt &elseStmt)
{
  auto entry = MakeLabel();
  auto exit = MakeLabel();

  EmitLabel(entry);
  LowerStmt(scope, elseStmt.GetStmt());
  EmitLabel(exit);
}
...
void Codegen::EmitDivide()
{
  assert(depth_ > 0 && "no elements on stack");
  depth_ -= 1;
  Emit<Opcode>(Opcode::DIVIDE);
}
// -----------------------------------------------------------------------------
